...
| LLM 질의 재작성 | Recall@5 0.691 → 0.788 (seed 도입 후 리포트 0.769). 구어와 문서 용어의 차이를 메운다 | 질문마다 LLM 호출 1회 추가(응답 지연 증가), 두 가지를 묻는 질문을 한쪽으로 좁히기도 함 |
...
- **비결정성**: seed 42 지정 후 검색 평가 2회의 재작성 질의가 36문항 모두 같았지만, API 제공사가 결정론을 보장하지는 않는다. 리포트에 문항별 재작성 질의와 답변을 남겨 추적한다.
