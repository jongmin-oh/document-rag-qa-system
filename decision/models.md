...
Gold Set이 준비되면 비교 모델 1개(voyage-4 또는 text-embedding-3-large)와 **검색 지표만** 비교해, 벤치마크 점수로 고른 선택을 우리 데이터로 확인한다. 청킹 실험(A/B/C) 동안에는 임베딩 모델을 바꾸지 않는다. 청킹과 임베딩을 동시에 바꾸면 효과를 구분할 수 없기 때문이다.
