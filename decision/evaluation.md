...
- Clarity: 법령을 모르는 사람이 한 번 읽고 자기 질문의 답을 이해할 수 있는가(결론 먼저, 용어 풀이, 불필요한 규정 나열 없음). 내용의 정확성과 분리해 전달 방식만 본다
...
상위 모델로 바꿨다. 같은 답변(커밋 `3afff43` 실행)을 `--judge-only`로 재채점해 모델 차이만 비교했다.
